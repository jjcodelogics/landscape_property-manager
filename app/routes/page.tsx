  ...
  return (
    <div className="min-h-screen pb-safe" style={{ background: 'var(--color-bg)' }}>
      ...
      >
        ...
        <h1 className="text-lg font-bold text-white flex-1">Points &amp; Routes</h1>
      </div>
      ...
      <div className="max-w-2xl mx-auto px-4 py-5 space-y-4">
        ...
        <div className="card overflow-hidden">
          ...
          {showPointForm && (
            <div className="p-5 space-y-3" style={{ borderBottom: '1px solid var(--color-border)', background: 'var(--color-surface-2)' }}>
              <div>
                <label className="block text-xs font-bold uppercase tracking-wider text-[var(--color-primary)] mb-2">Title *</label>
                <input value={pointTitle} onChange={(e) => setPointTitle(e.target.value)} className="input" placeholder="e.g. Bin A1" />
              </div>
              <div>
              ...
              <div>
                <label className="block text-xs font-bold uppercase tracking-wider text-[var(--color-primary)] mb-2">Notes</label>
                <textarea value={pointNotes} onChange={(e) => setPointNotes(e.target.value)} className="input resize-none" rows={2} placeholder="Optional notes…" />
              </div>
              ...
              <div className="flex gap-2">
                ...
                <button onClick={() => setShowPointForm(false)} className="btn btn-ghost text-sm">Cancel</button>
              </div>
            </div>
          )}

          {loading ? (
            <div className="px-5 py-8 text-center text-sm text-[var(--color-text-muted)]">Loading…</div>
          ) : points.length === 0 ? (
            <div className="px-5 py-8 text-center">
              <MapPin className="w-8 h-8 mx-auto mb-2 text-[var(--color-text-light)]" />
              <p className="text-[var(--color-text-light)] italic text-sm">No points yet.</p>
            </div>
          ) : (
            <ul>
        ...
        <div className="card overflow-hidden">
          ...
          {showRouteForm && (
            <div className="p-5 space-y-3" style={{ borderBottom: '1px solid var(--color-border)', background: 'var(--color-surface-2)' }}>
              <div>
                <label className="block text-xs font-bold uppercase tracking-wider text-[var(--color-primary)] mb-2">Route Name *</label>
                <input value={routeTitle} onChange={(e) => setRouteTitle(e.target.value)} className="input" placeholder="e.g. North Wing Bins" />
              </div>
              {points.length > 0 && (
                <div>
                  <label className="block text-xs font-bold uppercase tracking-wider text-[var(--color-primary)] mb-2">Select Points</label>
              ...
              <div className="flex gap-2">
                ...
                <button onClick={() => setShowRouteForm(false)} className="btn btn-ghost text-sm">Cancel</button>
              </div>
            </div>
          )}
          ...
          {loading ? (
            ...
            <div className="px-5 py-8 text-center">
              ...
              <p className="text-[var(--color-text-light)] italic text-sm">No routes yet. Create one by grouping points.</p>
            </div>
          ) : (
            <ul>
                    ...
                    <div className="flex items-start gap-3">
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-[var(--color-text-muted)] mb-1.5">
                          {route.point_ids.length} point{route.point_ids.length !== 1 ? 's' : ''}
                        </p>
